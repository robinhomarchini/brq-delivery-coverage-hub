...
- No Relatorio de Metas, a aba Clientes e uma visao derivada no grao Cliente + Ano. Ela consolida Hunters, Delivery/Farmers e Hunters Especializados a partir de `person_customer_assignments`/`people.clientIds`, `revenue_target_allocations`, `studio_target_allocations` e `specialist_hunter_studio_assignments`; nao cria nova fonte de verdade nem migration.
