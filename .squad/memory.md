...
- Na importação manual de baseline de Studio/Área, se a planilha trouxer `BU` ou `CC CROSS`, somente linhas `BU Financial` entram no snapshot; se a planilha não trouxer essa coluna, a tela exige confirmação manual de que a origem pertence à BU Financial antes de aceitar as linhas sem BU. Isso evita poluir a base com clientes de outras BUs e preserva as marcações Financial já salvas em snapshots anteriores.
