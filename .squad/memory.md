...
- Em 2026-07-22, duplicatas em `revenue_target_allocations` passaram a ser tratadas como risco operacional no grao Cliente + Pessoa + Tipo + Ano. A tela temporaria `/auditoria-duplicatas` lista duplicatas literais, suspeitas de Studio contido inflando Meta Squads/Times e Studios com valor sem meta total correspondente em Metas por Pessoa; nao remove automaticamente. A migration `20260722193000_prevent_duplicate_revenue_target_allocations.sql` bloqueia novas duplicatas por trigger ate a limpeza auditada permitir reforcar com constraint unica definitiva. Relatorios oficiais devem ser resilientes a duplicatas legadas e usar a linha canonica antes de descontar Studios contidos.
