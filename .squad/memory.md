...
- Rollups comuns do Relatório de Metas ficam em `src/lib/reports/person-target-rollups.ts`: Studio Hunter efetivo, renovação de Studio elegível por pessoa, Meta Squads/Times descontando herança e fallback de Hunter principal. Não duplicar esses cálculos em tela/export.
...
- No Relatório de Metas, a visão `Pessoas x Clientes` é o detalhe operacional no grão Pessoa + Cliente. Ela combina vínculos cadastrais, responsáveis Delivery/Farmer, Metas Squads/Times e heranças de Studio Hunter/Studio Manutenção, sem criar fonte de verdade nova.
...
- Planilha oficial Financial: para Pessoa + Cliente com uma ou mais linhas de Studio herdado, a linha sem Studio exibe Meta Squads/Times líquida (`Meta atual - Studio contido`). Linhas de Studio seguem separadas com o Studio em `Cliente Faturamento`. O subtotal da pessoa na planilha oficial soma Meta Squads/Times líquida + Studios herdados e deve representar a Meta atual total da pessoa. A regra vale igualmente para Hunter e Renovação/Manutenção, independentemente do cargo/perfil da pessoa.
- Planilha oficial Financial: o desconto de Studios contidos deve ser resiliente a legado com IDs diferentes para a mesma pessoa operacional. Quando a meta direta e a linha de Studio usam personIds diferentes, mas o nome normalizado é igual, a exportação agrupa esses IDs apenas para leitura e calcula a linha Meta Squads/Times líquida corretamente. Isso cobre casos como Ricardo Lucio do Bonfim com Renovação total e Studios Analytics/Salesforce sem tratar Studio contido normal como duplicata física.
...
- Dashboard Executivo / Comparativo Baseline: `Alocado em Pessoas` deve seguir a mesma base oficial da Planilha oficial. Hunter Especializado nao compoe KPIs oficiais, mesmo quando existe `revenue_target_allocations` historico ou Studio Hunter atribuido; esses valores ficam apenas na visao gerencial/sinalizacao especialista. Em 2026-07-23 o delta entre Dashboard e Planilha era R$ 2,8MM: Professional Services R$ 1,75MM + Intel R$ 50k em Hunter Especializado direto, e PISMO R$ 600k + CSU R$ 400k em Studio Hunter especialista.
...
- `npm run deploy:prod`: unico caminho aprovado para deploy de producao Vercel no Windows/Codex.
- `npm run deploy:inspect:prod`: unico caminho aprovado para inspecionar o alias principal de producao.
- `npm run deploy:inspect -- <deployment-url>`: caminho aprovado para inspecionar uma URL especifica de deployment.
...
- Na visao Clientes do Relatorio de Metas, listas de Hunters, Delivery/Farmers e Hunters Especializados exibem apenas nomes na tela e nas exportacoes. O modo "Com valores" adiciona colunas financeiras do cliente, nao valores anexados aos nomes das pessoas.
...
- Para deploy Vercel manual no Windows, sempre usar apenas os scripts versionados: `npm run deploy:check`, `npm run deploy:prod`, `npm run deploy:inspect:prod` e `npm run deploy:inspect -- <url>`. Eles passam por `scripts/vercel-cli.mjs`, que carrega `.env.production.local`/`.env.local`, usa Node 22, Vercel CLI fixo, cache `.npm-cache` e `LOCALAPPDATA=.vercel-cli/localappdata`. Nao usar `npx vercel ...` direto nem variantes soltas de inspect/deploy.
- Se antivirus bloquear `%USERPROFILE%\.vercel\auth.json`, usar `VERCEL_TOKEN` em `.env.local`. Os scripts `deploy:*` carregam `.env.local` via `scripts/env-loader.mjs`; nunca commitar token nem colar o valor em resposta.
...
- Na tela Clientes, salvar metas gerais do cliente nao pode sobrescrever metas ja lancadas em Metas por Pessoa para Hunters, Farmers ou Delivery associados. A tela pode sincronizar vinculo de Hunter responsavel com valor zero quando nao houver alocacao, mas se ja houver alocacao da pessoa/cliente/ano ela deve ser preservada. A conciliacao visual usa vermelho para abaixo, azul para batido, verde para acima e roxo para clientes cobertos apenas por Hunter Especializado.
- Na tela Clientes, cobertura apenas por Hunter Especializado fica roxa mesmo quando a meta esta batida. Alocacoes zeradas de Farmer/Delivery nao descaracterizam essa cobertura. Cliente normal apenas com meta Hunter, sem Farmer/Delivery, fica azul quando a meta esta reconciliada.
...
- Em 2026-07-28, os papeis persistidos foram centralizados em `src/lib/roles.ts` com `ROLE_TYPES` como fonte TypeScript canonica. `Manager` continua sendo apenas label/relacao, nunca `RoleType` persistido. `npm run test:roles` valida `roles.ts`, `validation.ts` e a migration canonica `20260707203000_specialist_hunter_role.sql`; nao voltar a validar roles apenas por tokens soltos.
- O organograma canonico e `src/components/organization/organization-chart.tsx` com `PersonCard`; `OrganizationChartV2`, seus cards dedicados e `use-organization-tree` foram removidos. Nao reintroduzir uma segunda implementacao de organograma.
- Hunter Especializado pode ter `assigned_amount` opcional em `specialist_hunter_studio_assignments`. E um override gerencial/editavel por linha de Studio, usado somente na tela/relatorio do Hunter Especializado; vazio usa o valor do Studio. Nao cria `revenue_target_allocations`, nao altera meta do cliente, pessoa, Studio, dashboard oficial nem baseline.
- A partir de 2026-07-21, `hunter_viewer` nao e mais estritamente read-only: o escopo e calculado por e-mail da Pessoa via `src/lib/hunter-access-scope.ts`. Pode ver Dashboard/Clientes/Relatorio filtrados aos clientes relacionados, editar apenas seus dados basicos em Pessoas, criar novo Cliente vinculado ao proprio Hunter e criar/editar/excluir Metas por Area/Studio somente quando o Hunter ou responsavel de Manutencao for a propria pessoa. A seguranca deve continuar no BFF/RLS: `createDeliveryCommandClient` exige `allowHunterScopedWrite` explicito e a migration `20260721103000_hunter_scoped_access.sql` adiciona helpers/policies/RPC branches para impedir writes fora do escopo.
- Simulacao de usuario para admin fica no cabecalho principal via `AccessContext` efetivo. Nao mexe no login, token Supabase, auditoria ou RLS; e recurso de QA/operacao para validar filtros/navegacao como outro usuario. Para validar seguranca real, continuar usando usuarios reais ou smokes RLS.
- No Dashboard Executivo em contexto `hunter_viewer`, os KPIs financeiros de Board precisam usar somente os clientes do escopo do Hunter; nao voltar a usar `getBoardTargetBaselineTotals` global nessa visao. Em Metas por Area/Studio, Hunter so pode editar linha inteira quando todos os segmentos com valor da linha pertencem a ele; linhas com Studio Hunter ou Manutencao de outro responsavel ficam somente leitura.
- No Comparativo Baseline em contexto `hunter_viewer`, a rota fica disponivel como consulta e deve usar o mesmo escopo de clientes da pessoa logada/simulada por `buildHunterAccessScope`. Board vs Cadastro, Baseline de Studios, KPIs e exportacoes devem ser filtrados aos clientes do Hunter; a acao de atualizar metas do cliente permanece indisponivel para esse perfil.
- No Comparativo Baseline > Board vs Cadastro, existem tres referencias distintas: Baseline Board, Cadastro do Cliente (`customer_target_years`/`customers`) e Alocado em Pessoas (`revenue_target_allocations` + `studio_target_allocations` contidos). O status principal e a acao de update continuam comparando Cadastro do Cliente x Baseline; Pessoas x Baseline aparece como reconciliacao operacional e deve ser exportado separadamente.
- Em 2026-07-22, duplicatas em `revenue_target_allocations` passaram a ser tratadas como risco operacional no grao Cliente + Pessoa + Tipo + Ano. A tela temporaria `/auditoria-duplicatas` lista registros fisicos repetidos e Studios com valor sem meta total correspondente em Metas por Pessoa; Studio contido normal nao e duplicata e apenas decompõe Meta Squads/Times nos relatorios. A tela permite excluir manualmente cada registro fisico repetido apos confirmacao. A migration `20260722193000_prevent_duplicate_revenue_target_allocations.sql` bloqueia novas duplicatas por trigger ate a limpeza auditada permitir reforcar com constraint unica definitiva. Relatorios oficiais devem ser resilientes a duplicatas legadas e usar a linha canonica antes de descontar Studios contidos.
