...
export interface PersonTargetReportTableProps {
  rows: PeopleReportRow[];
  selectedPersonIds: ReturnType<typeof useSetSelection>;
  ...
  onRowDoubleClick: (personId: string) => void;
  canEdit: boolean;
  year: number;
}
...
export function PersonTargetReportTable({
  rows,
  selectedPersonIds,
  allVisibleSelected,
  sort,
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow
                ...
                onDoubleClick={() => onRowDoubleClick(row.personId)}
              >
