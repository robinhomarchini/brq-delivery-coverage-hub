...
function findTargetHeaderRowIndex(rows: SpreadsheetCell[][]) {
  const headerCandidates = rows.reduce<number[]>((candidates, row, rowIndex) => {
    const headers = row.map((cell) => normalizeHeader(String(cell ?? "")));
    const isTargetHeader = findOptionalHeaderIndex(headers, requiredHeaders.customerName) >= 0
    ...
    if (isTargetHeader) candidates.push(rowIndex);
    return candidates;
  }, []);

  const index = headerCandidates.at(-1);
  if (index !== undefined) return index;
  throw new Error(`Cabeçalho da tabela Financial da baseline não encontrado. Verifique o segundo quadro da aba Resumo RL 2026, a partir da linha ${officialFinancialCurveTableStartRow}.`);
}
