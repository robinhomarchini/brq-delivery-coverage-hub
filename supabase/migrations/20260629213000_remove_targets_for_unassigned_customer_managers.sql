-- Keep customer manager assignments and farmer/renewal targets consistent.
--
-- When a manager is removed from a customer, their Renewal + Expansion target
-- allocation for that customer must be removed too. The customer target remains
-- unchanged and becomes an open/pending allocation amount.
  ...
  where allocation.customer_id = p_id
    ...
    and exists (
      ...
      where assignment.customer_id = p_id
        and assignment.person_id = allocation.person_id
        and p.is_manager
    )
    and not exists (
      select 1
