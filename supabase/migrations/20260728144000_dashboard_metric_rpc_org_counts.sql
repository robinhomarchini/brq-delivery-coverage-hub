-- Extend executive dashboard summary with verified organizational counts.
--
-- Adds active_people_count, director_count and manager_count to the existing
-- get_executive_dashboard_summary RPC. These counts are scoped to the same
-- hunter/access filter applied to the financial metrics.
--
-- Hunter scope includes the hunter person explicitly (p_hunter_person_id)
-- so that a hunter with assignments is always reflected in the counts even
-- when assignments are not yet persisted.
--
-- SECURITY INVOKER remains; RLS on people and assignment tables continues to
-- apply. No new grants or indexes created; reuse existing indexes on
-- customer_id/target_year/person_id where applicable.
