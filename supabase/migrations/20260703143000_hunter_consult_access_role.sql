-- Add a restricted consulting role for Hunters.
--
-- hunter_viewer is an active BRQ user who can access only read-only Hunter
-- scoped views in the app. It is intentionally not included in
-- can_edit_delivery_data(), so database writes remain limited to editor/admin.
...
alter table public.app_users
  ...
  check (role in ('viewer', 'hunter_viewer', 'editor', 'admin'));

alter table public.app_access_invites
  drop constraint if exists app_access_invites_role_check;

alter table public.app_access_invites
  add constraint app_access_invites_role_check
  check (role in ('viewer', 'hunter_viewer', 'editor', 'admin'));
