...
Clientes New Logo que ficam em controle, mas não compõem a meta oficial do ano,
devem usar o mesmo grão anual, com `counts_toward_target = false` e um motivo
de exclusão. Esses clientes permanecem visíveis no cadastro e no controle
operacional, mas a meta efetiva para batimento do ano é zero. Relatório de
Metas e Dashboard Executivo devem iniciar sem New Logos e oferecer opção
explícita para incluí-los na consulta/exportação.
...
cliente/studio.
O batimento de Studios deve comparar nomes equivalentes de alianças por chave
...
Studios com valor sem meta total correspondente em Metas por Pessoa. Studio
contido em meta total existente é decomposição normal para relatório oficial e
não deve aparecer como duplicata ou recomendação de remoção.
